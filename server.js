...
const commands = {
	'play': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`İlk önce ${ayarlar.prefix}ekle <Şarkı Adı/URL> Komutu İle Şarkı Ekleyin!`);
		...
		(function play(song) {
			...
			msg.channel.sendMessage(`Çalınan Şarkı: **${song.title}** Şarkıyı Açan: **${song.requester}**`);
	...
	'add': async (msg) => {
		...
		if (url == '' || url === undefined) return msg.channel.sendMessage(`Bir YouTube Linki Eklemek İçin ${ayarlar.prefix}add <URL> yazınız`);
	...
	'queue': (msg) => {
		if (queue[msg.guild.id] === undefined) return msg.channel.sendMessage(`Sıraya ilk Önce Şarkı Ekle: ${ayarlar.prefix}add`);
		let tosend = [];
